...
# How long a panel keeps working after the last SUCCESSFUL online check.
# Not a grace period on the licence itself - an expired licence stops
# immediately. This only covers "we could not ask".
GRACE_DAYS = 7
...
def verify(
    ...
    *,
    ...
    # The licence itself is fine. The only remaining question is whether we
    # have been out of touch with the licence server for too long.
    grace_days_left = None
    if last_online_check is not None:
        offline_for = now - last_online_check
        if offline_for > dt.timedelta(days=GRACE_DAYS):
            return LicenseStatus(
                False, REASON_GRACE_EXHAUSTED, payload=payload,
                message=_MESSAGES[REASON_GRACE_EXHAUSTED].format(days=GRACE_DAYS),
                days_left=days_left,
            )
        if offline_for > dt.timedelta(hours=24):
            grace_days_left = GRACE_DAYS - offline_for.days
