...
npx @Codex-flow/cli@latest init --hooks
```
...
npx @Codex-flow/cli@latest hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx @Codex-flow/cli@latest hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx @Codex-flow/cli@latest hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
npx @Codex-flow/cli@latest hook pre-edit [options]
...
Examples:
  npx @Codex-flow/cli@latest hook pre-edit --file "src/auth/login.js"
  npx @Codex-flow/cli@latest hook pre-edit -f "config/db.js" --validate-syntax
  npx @Codex-flow/cli@latest hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx @Codex-flow/cli@latest hook pre-bash --command <cmd>
...
Examples:
  npx @Codex-flow/cli@latest hook pre-bash -c "rm -rf /tmp/cache"
  npx @Codex-flow/cli@latest hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx @Codex-flow/cli@latest hook pre-task [options]
...
Examples:
  npx @Codex-flow/cli@latest hook pre-task --description "Implement user authentication"
  npx @Codex-flow/cli@latest hook pre-task -d "Continue API dev" --load-memory
  npx @Codex-flow/cli@latest hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx @Codex-flow/cli@latest hook pre-search --query <query>
...
Examples:
  npx @Codex-flow/cli@latest hook pre-search -q "authentication middleware"
```
...
```bash
npx @Codex-flow/cli@latest hook post-edit [options]
...
Examples:
  npx @Codex-flow/cli@latest hook post-edit --file "src/components/Button.jsx"
  npx @Codex-flow/cli@latest hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx @Codex-flow/cli@latest hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx @Codex-flow/cli@latest hook post-bash --command <cmd>
...
Examples:
  npx @Codex-flow/cli@latest hook post-bash -c "npm test" --update-metrics
```
...
```bash
npx @Codex-flow/cli@latest hook post-task [options]
...
Examples:
  npx @Codex-flow/cli@latest hook post-task --task-id "auth-implementation"
  npx @Codex-flow/cli@latest hook post-task -t "api-refactor" --analyze-performance
  npx @Codex-flow/cli@latest hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx @Codex-flow/cli@latest hook post-search --query <query> --results <path>
...
Examples:
  npx @Codex-flow/cli@latest hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx @Codex-flow/cli@latest hook mcp-initialized --swarm-id <id>
...
```bash
npx @Codex-flow/cli@latest hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx @Codex-flow/cli@latest hook task-orchestrated --task-id <id>
...
```bash
npx @Codex-flow/cli@latest hook neural-trained --pattern <name>
...
```bash
npx @Codex-flow/cli@latest hook memory-sync --namespace <ns>
...
```bash
npx @Codex-flow/cli@latest hook session-start --session-id <id>
...
```bash
npx @Codex-flow/cli@latest hook session-restore --session-id <id>
...
Examples:
  npx @Codex-flow/cli@latest hook session-restore --session-id "swarm-20241019"
  npx @Codex-flow/cli@latest hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
npx @Codex-flow/cli@latest hook session-end [options]
...
Examples:
  npx @Codex-flow/cli@latest hook session-end --session-id "dev-session-2024"
  npx @Codex-flow/cli@latest hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx @Codex-flow/cli@latest hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx @Codex-flow/cli@latest hook notify --message <msg>
...
Examples:
  npx @Codex-flow/cli@latest hook notify -m "Task completed" --level info
  npx @Codex-flow/cli@latest hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx @Codex-flow/cli@latest hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx @Codex-flow/cli@latest hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx @Codex-flow/cli@latest hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx @Codex-flow/cli@latest hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx @Codex-flow/cli@latest hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx @Codex-flow/cli@latest hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx @Codex-flow/cli@latest hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx @Codex-flow/cli@latest hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx @Codex-flow/cli@latest hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx @Codex-flow/cli@latest hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx @Codex-flow/cli@latest hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx @Codex-flow/cli@latest hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx @Codex-flow/cli@latest hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx @Codex-flow/cli@latest hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx @Codex-flow/cli@latest hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
npx @Codex-flow/cli@latest hook session-end --session-id "dev-2024"
...
for FILE in $FILES; do
  ...
  npx @Codex-flow/cli@latest hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx @Codex-flow/cli@latest hook post-edit --file "$FILE" --auto-format
done
...
npx @Codex-flow/cli@latest hook notify \
...
# Run quality checks
npx @Codex-flow/cli@latest hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx @Codex-flow/cli@latest metrics score --format json | jq -r '.truth_score')
...
npx @Codex-flow/cli@latest hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx @Codex-flow/cli@latest hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx @Codex-flow/cli@latest hook post-edit \
  --file "api/auth.js" \
...
npx @Codex-flow/cli@latest hook post-task \
  --task-id "auth-api" \
  --analyze-performance \
  --store-decisions \
  --export-learnings
...
npx @Codex-flow/cli@latest hook session-restore \
...
npx @Codex-flow/cli@latest hook session-start --session-id "fullstack-feature"
...
# Backend work
npx @Codex-flow/cli@latest hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx @Codex-flow/cli@latest hook post-edit \
  --file "api/profile.js" \
...
npx @Codex-flow/cli@latest hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx @Codex-flow/cli@latest hook post-edit \
...
npx @Codex-flow/cli@latest hook session-start --session-id "debug-memory-leak"
...
npx @Codex-flow/cli@latest hook pre-search --query "EventEmitter"
# ... search executes ...
npx @Codex-flow/cli@latest hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx @Codex-flow/cli@latest hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx @Codex-flow/cli@latest hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx @Codex-flow/cli@latest hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx @Codex-flow/cli@latest hook post-edit \
    --file "$file" \
...
npx @Codex-flow/cli@latest hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .Codex-flow/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx @Codex-flow/cli@latest hook validate-config
```
...
- `npx @Codex-flow/cli@latest init --hooks` - Initialize hooks system
- `npx @Codex-flow/cli@latest hook --list` - List available hooks
- `npx @Codex-flow/cli@latest hook --test <hook>` - Test specific hook
- `npx @Codex-flow/cli@latest memory usage` - Manage memory
- `npx @Codex-flow/cli@latest agent spawn` - Spawn agents
- `npx @Codex-flow/cli@latest swarm init` - Initialize swarm
