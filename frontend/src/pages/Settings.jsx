...
// BotSaleChargeCard removed (2026-09-05): bot sales now always charge the
// reseller's credit - see backend/app/routers/bot.py's _charge_seller -
    ...
    <Layout>
      ...
      <div className="card mb-4">
        ...
        <form onSubmit={submitBot} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* Two different mechanisms, side by side and labelled as such -
              the reverse proxy replaces the API ADDRESS, the SOCKS/HTTP one
              TUNNELS to the real address. Mixing them up is the likeliest
              way to end up with a bot that starts and then receives
              nothing. */}
          <div className="md:col-span-2">
          ...
          {/* The «راه‌اندازی خودکار پروکسی روی نود میکروتیک» card used to sit
              here. Removed at the owner's request: it never actually produced
              a working proxy, so it was a button that promised a result it
              could not deliver. The two fields above still accept a proxy
              URL typed in by hand, which is the path that does work. */}
          <div>
