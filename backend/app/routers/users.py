...
def bulk_create_users(
    ...
    if payload.package_id:
        ...
        admin_billing.charge_for_package(db, admin, package, units=payload.count)
    ...
    try:
        ...
        if package:
            admin_billing.refund_for_package(db, admin, package, units=payload.count)
        raise
    if package and result["created_count"] < payload.count:
        admin_billing.refund_for_package(db, admin, package, units=payload.count - result["created_count"])
    return result
