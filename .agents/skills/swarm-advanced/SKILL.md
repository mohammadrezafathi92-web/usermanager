...
npx @Codex-flow/cli@latest swarm "research AI trends in 2025" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel \
...
npx @Codex-flow/cli@latest swarm "build REST API with authentication" \
...
```bash
# Quick testing swarm
npx @Codex-flow/cli@latest swarm "test application comprehensively" \
  --strategy testing \
  --mode star \
  --parallel \
  --timeout 600
